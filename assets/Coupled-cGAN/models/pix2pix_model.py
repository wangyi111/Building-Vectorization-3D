...
#
#                pdb.set_trace()
#                self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG.parameters(), list(self.task_weights.values()), list(self.criterionL1.parameters())),
#                                                    lr=opt.lr, betas=(opt.beta1, 0.999))                    
        ...
        if self.isTrain:
          ...
          if not os.path.exists(opt.Out[0]):
              os.makedirs(opt.Out[0])     
                 
          f = open( opt.Out[0] + '/net.txt', 'w' )
          f.write( str(self.netG) )
          f.close()
    ...
    def backward_G(self):
        ...
        if self.opt.loss_weights:           
            ...
            # Second, G(A) = B
            if self.opt.adaptive:
                pdb.set_trace()
                #self.loss_G_L1 = torch.mean(self.criterionL1.lossfun((self.fake_B - self.real_B)[:,None]))
            else:
