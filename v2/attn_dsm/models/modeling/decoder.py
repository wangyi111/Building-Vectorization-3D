...
'''
A modification of deeplabv3+
    A decoder that takes information from feature maps of various sizes
    decoder is xception 71
'''
...
class Decoder(nn.Module):
    def __init__(self, num_classes, backbone, BatchNorm, multi_scale_upsample = True):
        ...
        if backbone == 'resnet' or backbone == 'drn':
            ...
            if multi_scale_upsample:
                low_level_inplanes = [64,128,256]  ##1/2, 1/4 1/8
            else:
   ...
   #### 1X1 conv to reduce num of channels for skip connection
        ...
        self.skip = [self.skip1_2, self.skip1_4, self.skip1_8]
        ...
        self.last_convs = [self.last_conv2, self.last_conv4, self.last_conv8]
    ...
    def forward(self, x, low_level_feat):#x, is output after aspp (256 channel), low_level_feat is a list of low level features from entryflow conv2, block 1, 3, 13 (1/2, 1/4, 1/8, 1/16)
        ...
        for i in range(len(self.last_convs)):
            x = F.interpolate(x, size=low_level_feat_reduced[-i-1].size()[2:], mode='bilinear', align_corners=True)
