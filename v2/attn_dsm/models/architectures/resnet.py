...
class CoupledUNetupperResnet_2(nn.Module): ## new!
    ...
    def __init__(self, n_classes=1, m_classes=3, backbone = "resnet50"):
        super(CoupledUNetupperResnet_3, self).__init__()
        ...
        for i in range(5):
            in_cl = 1024/(2**i)
            scale_factor = 2**(4-i)
            sb = nn.Sequential(nn.Conv2d(in_cl, 3, kernel_size=1, stride=1),
                               nn.Upsample(mode='bilinear', scale_factor=scale_factor))
            side_branch.append(sb)
        self.side_branch = nn.ModuleList(side_branch)

       
        self.out_fuse = nn.Conv2d(15, m_classes, kernel_size=1, stride=1)        
    ...
    def forward(self, *input):
        ...
        ####################  DSM Branch  ###############################
        dsm_pre_pools = dict()
        dsm_pre_pools["dsm_layer_0"] = input[0] # (B,1,256,256) dsm
        ...
        ####################  IMG Branch  ###############################
        img_pre_pools = dict()
        img_pre_pools["img_layer_0"] = input[1] # (B,1,256,256) ortho
        ...
        for i, block in enumerate(self.up_blocks, 1):
            ...
            #pdb.set_trace()
            self.out_sides.append(self.side_branch[i-1](x))
        ...
        x2 = torch.cat((self.out_sides[0],self.out_sides[1],self.out_sides[2],self.out_sides[3],self.out_sides[4]),1)
        x2 = self.out_fuse(x2)

        del dsm_pre_pools, img_pre_pools  
        
        return self.out_sides[0],self.out_sides[1],self.out_sides[2],self.out_sides[3],self.out_sides[4],x2
...
''' attn + HED edges'''                
class CoupledUNetupperResnet_3(nn.Module): ## new!
    DEPTH = 6
    ...
    def __init__(self, n_classes=1, m_classes=3, backbone = "resnet50"):
        ...
        # attention 3
        self.ca3 = ChannelAttention(2048,64)
        ...
        self.out_corner = nn.Conv2d(64, 2, kernel_size=1, stride=1)   
        
        self.out_edges = nn.Conv2d(64, 2, kernel_size=1, stride=1)
        #self.softmax = nn.Softmax()
        ...
        for i in range(5):
            in_cl = 1024/(2**i)
            scale_factor = 2**(4-i)
            sb = nn.Sequential(nn.Conv2d(in_cl, 2, kernel_size=1, stride=1),
                               nn.Upsample(mode='bilinear', scale_factor=scale_factor))
            side_branch.append(sb)
        self.side_branch = nn.ModuleList(side_branch)

       
        self.out_fuse = nn.Conv2d(6, 2, kernel_size=1, stride=1)        
    ...
    def forward(self, *input):
        ...
        # attention 1
        dsm = self.ca0(dsm) * dsm
        dsm = self.sa0(dsm) * dsm
        ...
        # attention 2
        img = self.ca1(img) * img
        img = self.sa1(img) * img
        ...
        x2 = self.out_corner(x)

        x3 = self.out_edges(x)
        #x2 = self.softmax(x2)

        fuse = torch.cat((self.out_sides[3],self.out_sides[4],x3),1)
        fuse = self.out_fuse(fuse)

        del dsm_pre_pools, img_pre_pools  
        
        return x1, x2, self.out_sides[3],self.out_sides[4],x3,fuse
...
''' attn '''
class CoupledUNetupperResnet_4(nn.Module): ## new!
    DEPTH = 6

    def __init__(self, n_classes=1, m_classes=3, backbone = "resnet50"):
        super(CoupledUNetupperResnet_4, self).__init__()
        ...
        self.out_dsm = nn.Conv2d(64, 1, kernel_size=1, stride=1)        
        self.tanh = nn.Tanh()
        
        #self.out_edges = nn.Conv2d(64, 3, kernel_size=1, stride=1)
        #self.softmax = nn.Softmax()       
