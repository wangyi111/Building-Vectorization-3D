        ...
        if total_steps % opt.val_freq == 0:
            ...
            with torch.no_grad():
                ...
                for data in tqdm(val_loader, desc="Validation   "):
                    ...
                    pred_DSM = model.netG.forward(*sample)
                    #pred_E = (torch.nn.Softmax(dim=1)(side1) + torch.nn.Softmax(dim=1)(side2) + torch.nn.Softmax(dim=1)(side3))/6 + torch.nn.Softmax(dim=1)(fuse)/2
                    #pdb.set_trace()
                    #pred_E = torch.nn.Softmax(dim=1)(pred_E)

                    #pred_C = torch.nn.Softmax(dim=1)(pred_C)

                    dsm_RMSE += metrics.RMSE(pred_DSM, data["B"].cuda())
	            dsm_MAE += metrics.MAE(pred_DSM, data["B"].cuda())
                    '''
	            #target_E = torch.argmax(data["Edges"],dim=1)
                    target_E = data["Edges"].new_zeros(data["Edges"][:,0,:,:].shape)
                    target_E[data["Edges"][:,2,:,:]>0] = 2 # background
                    target_E[data["Edges"][:,0,:,:]>0] = 0 # corner -> edge
