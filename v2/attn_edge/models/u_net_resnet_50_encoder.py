...
class UNetWithResnet50Encoder(nn.Module):
    ...
    def __init__(self, n_classes=1):
        ...
        self.input_conv = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
#        layer.weight.data.copy_(onechannel_weight)
    ...
    def forward(self, x, with_output_feature_map=False):
        ...
        #pdb.set_trace()
        for i, block in enumerate(self.up_blocks, 1):
            key = "layer_{0}".format(str(UNetWithResnet50Encoder.DEPTH - 1 - i))
            #key = "layer_{UNetWithResnet50Encoder.DEPTH - 1 - i}"
            x = block(x, pre_pools[key])
...
class CoupledUNetResnet(nn.Module):
    DEPTH = 6

    def __init__(self, n_classes=1):
        super(CoupledUNetResnet, self).__init__()
        #resnet = torchvision.models.resnet.resnet152(pretrained=False)
        #resnet = torchvision.models.resnet.resnet101(pretrained=False)
        resnet = torchvision.models.resnet.resnet50(pretrained=False)
        
        print "from_u_net_resnet_50"
        ...
        up_blocks.append(UpBlockForUNetWithResNet50(in_channels=128 + 64, out_channels=128,
                                                    up_conv_in_channels=256, up_conv_out_channels=128))
        up_blocks.append(UpBlockForUNetWithResNet50(in_channels=64 + 2, out_channels=64,
                                                    up_conv_in_channels=128, up_conv_out_channels=64))
...
#        layer.weight.data.copy_(onechannel_weight)
    ...
    def forward(self, *input):
        ...
        for i, block in enumerate(self.up_blocks, 1):
            dsm_key = "dsm_layer_{0}".format(str(CoupledUNetResnet.DEPTH - 1 - i))
            img_key = "img_layer_{0}".format(str(CoupledUNetResnet.DEPTH - 1 - i))
            key = "layer_{0}".format(str(CoupledUNetResnet.DEPTH - 1 - i))
...
class CoupledUNetLowerResnet(nn.Module):
    ...
    def __init__(self, n_classes=1, backbone = "resnet18"):
        ...
        up_blocks.append(UpBlockForUNetWithResNet50(in_channels=128 + 64, out_channels=64,
                                                    up_conv_in_channels=128, up_conv_out_channels=64))
        up_blocks.append(UpBlockForUNetWithResNet50(in_channels=64 + 2, out_channels=64,
...
#        layer.weight.data.copy_(onechannel_weight)
    ...
    def forward(self, *input):
        ...
        for i, block in enumerate(self.up_blocks, 1):
            ...
            key = "layer_{0}".format(str(CoupledUNetLowerResnet.DEPTH - 1 - i))
...
### Version 1          
#model = Coupled_UNetWithResnet50Encoder().cuda()
#inp1 = torch.rand((2, 1, 512, 512)).cuda()
#inp2 = torch.rand((2, 1, 512, 512)).cuda()
#out1, pre_pools1 = model(inp1)
#out2, pre_pools2 = model(inp2)
#
#inp = []
#inp.append(out1)
#inp.append(out2)
#
#pre_pools = [pre_pools1,pre_pools2]
#
#decoder = Coupled_UNetWithResnet50Decoder().cuda()

#out = decoder(pre_pools, *inp)

### Version 2

#model = Coupled_UNetResNet50().cuda()
#inp1 = torch.rand((2, 1, 512, 512)).cuda()
#inp2 = torch.rand((2, 1, 512, 512)).cuda()
#
#inp = []
...
#### Version 4 - Test "UNetWithResnet50Encoder"
...
### Test "CoupledUNetLowerResnet"
#model = CoupledUNetLowerResnet().cuda()
#print model
#model.cuda()
#inp1 = torch.rand((2, 1, 256, 256)).cuda()
#inp2 = torch.rand((2, 1, 256, 256)).cuda()
