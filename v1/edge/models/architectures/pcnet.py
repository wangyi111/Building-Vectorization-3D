...
class AbstractModel(nn.Module):
    ...
    def set_input(self, input):
        ...
        # Input 
        self.input_dsm = input["input_dsm"]
        # Output
        self.gt_model = input["gt_model"]

        if "input_ortho" in input:
            input_ortho = input['Ortho']
            self.input_ortho = self.Tensor(input_ortho.size()).copy_(input_ortho)

            
        if 'gt_mask' in input:
            gt_mask = input['gt_mask']
            self.gt_mask = self.Tensor(gt_mask.size()).copy_(gt_mask)
...
class ConvUNet(nn.Module):
    def __init__(self, layer_size=7, input_channels=1, upsampling_mode='nearest'):
        ...
        self.upsampling_mode = upsampling_mode
        self.layer_size = layer_size
        self.enc_1 = Activ(input_channels, 64, bn=False, sample='down-7')
        self.enc_2 = Activ(64, 128, sample='down-5')
        self.enc_3 = Activ(128, 256, sample='down-5')
        self.enc_4 = Activ(256, 512, sample='down-3')
        for i in range(4, self.layer_size):
            name = 'enc_{:d}'.format(i + 1)
            setattr(self, name, Activ(512, 512, sample='down-3'))

        for i in range(4, self.layer_size):
            name = 'dec_{:d}'.format(i + 1)
            setattr(self, name, Activ(512 + 512, 512, activ='leaky'))
        self.dec_4 = Activ(512 + 256, 256, activ='leaky')
        self.dec_3 = Activ(256 + 128, 128, activ='leaky')
        self.dec_2 = Activ(128 + 64, 64, activ='leaky')
        self.dec_1 = Activ(64 + input_channels, input_channels,
                              bn=False, activ=None, conv_bias=True)
