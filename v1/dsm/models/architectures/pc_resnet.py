...
class UpBlockForUNetWithResNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, up_conv_in_channels=None, up_conv_out_channels=None,
        ...
        self.pconv = PartialConv2d(up_conv_in_channels, up_conv_out_channels, padding=0, kernel_size=1, stride=1, multi_channel=True, return_mask=True)
    ...
    def forward(self, up_x, down_x, up_mask, down_mask):
        """
        ...
        mask = self.upsample(up_mask)
               
        x, mask = self.pconv(x, mask)

        x = torch.cat([x, down_x], 1)
        mask = torch.cat([mask, down_mask], 1)

        #pdb.set_trace()
                
        x, mask = self.conv_block_1([x, mask])
        x, mask = self.conv_block_2([x, mask])
...
class PConvUResNet(nn.Module):
    DEPTH = 6

    def __init__(self, n_classes=1):
        super(PConvUResNet, self).__init__()
        ...
        up_blocks.append(UpBlockForUNetWithResNet(2048, 1024)) #2048
        up_blocks.append(UpBlockForUNetWithResNet(1024, 512)) #1024
        ...
        self.out = PartialConv2d(64, n_classes, kernel_size=1, stride=1, padding=0,
                               bias=False, multi_channel=True, return_mask=True)
...
#        layer.weight.data.copy_(onechannel_weight)
    ...
    def forward(self, *input):
        ...
        pre_pools["layer_0"] = input[0]
        pre_mask_pools["layer_0"] = input[1]
                
        x, mask = self.input_conv(input[0], input[1])
        x = self.input_block(x)
        pre_pools["layer_1"] = x
        pre_mask_pools["layer_1"] = mask
        
        x, indices = self.input_pool(x)

        # retrieve corresponding elements from upper mask according to indices
        mask = retrieve_elements_from_indices(mask, indices)

        
        for i, block in enumerate(self.down_blocks, 2):
            x, mask = block([x,mask])
            if i == (PConvUResNet.DEPTH - 1):
                continue
            #pre_pools["layer_{i}"] = x
            pre_pools["layer_{0}".format(str(i))] = x
            pre_mask_pools["layer_{0}".format(str(i))] = mask
        
        x, mask = self.bridge(x, mask)

        for i, block in enumerate(self.up_blocks, 1):
            key = "layer_{0}".format(str(PConvUResNet.DEPTH - 1 - i))
            #key = "layer_{UNetWithResnet50Encoder.DEPTH - 1 - i}"
            #pdb.set_trace()
            x, mask = block(x, pre_pools[key], mask, pre_mask_pools[key])
