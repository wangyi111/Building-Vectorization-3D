    ...
    for data in tqdm(train_loader, desc="Current Epoch"): # iterate training data
        ...
        if total_steps % opt.display_freq == 0:
            ...
            for name,param in model.netD.named_parameters():
                tb_writer.add_histogram(name,param,global_step=total_steps)
...
############################# stop here ############################################### 
        ...
        if total_steps % opt.val_freq == 0:
            ...
            with torch.no_grad():
                ...
                val_loss = {}
                val_loss['dsm_RMSE'] = val_result_DSM
                val_loss['dsm_MAE'] = dsm_MAE
                tb_writer.add_scalars('val_loss',val_loss,global_step=total_steps,walltime=None)

                # output results
                logger.info("---------- Validation Results ----------")
                logger.info("RMSE_DSM: %f", val_result_DSM)
